...
functions.cloudEvent('helloPubSub', cloudEvent => {
    ...
    console.log(`[EMAIL]
        Subject: ${paid ? "Order Confirmation – Your Shipment is on the Way" : "Reminder: Your Cart is Waiting!"}
        
        Dear Customer ${customerId},
        
        ${paid
            ? `Thank you for your order! Your payment has been received, and your cart (${items.map(item => item.itemId).join(', ')}) is being processed for shipment.`
            : `You still have items in your cart (${items.map(item => item.itemId).join(', ')}). Complete your checkout soon to ensure availability!`
        }
        
        Priority Level: ${priority}
        Time Spent on Site: ${timeOnSite} seconds
